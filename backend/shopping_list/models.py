...
class Category(models.Model):
    ...
    def can_be_deleted_by(self, user):
        ...
        # Системные категории нельзя удалять НИКОМУ (даже админам)
        if self.is_system_category():
            return False
        # Категории, созданные пользователем, можно удалять только создателю
        if self.created_by == user:
            return True
        # Если created_by не указан (старые категории), разрешаем удаление только админам
        # НО только если это НЕ системная категория (проверка выше уже это исключила)
